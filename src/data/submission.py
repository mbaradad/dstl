...
def generate_submission(classifier, generate_images=False):
  ...
  for idx in range(len(d.get_image_list())):
    ...
    if idx < 4 and generate_images:
      plt.imshow(np.transpose(d.generate_one(idx)[0][0:3], [1, 2, 0]))
      plt.savefig(images_dir +'/img_' + str(idx) + '.jpg')
    for i in range(0, len(positions_by_idx[idx]), chunk_size):
      ...
      i = 0
      for pos in actual_pos:
        masks[:, pos[1]:(pos[1]+crop_size[0]), pos[2]:(pos[2]+crop_size[1])] = predictions[i] > 0.5
        i+=1
    ...
    for i in range(len(masks)):
      ...
      if idx < 4 and generate_images:
        ...
        plt.savefig(images_dir + '/img_' + str(idx) + '_m_' + str(i) + '_' + d.class_id_to_name(i) + 'opening.jpg')
      print 'Computing pols'
      polygons = mask_to_polygons_rasterio(actual_mask)
      print 'End computing pols'
